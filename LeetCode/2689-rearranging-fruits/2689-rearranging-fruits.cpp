...
public:
    long long minCost(vector<int>& basket1, vector<int>& basket2) {
        ...
        for (int x : basket2) freq[x]++;//freq to check even for each fruit 
        ...
        for (int x : basket1) count1[x]++;//count for basket 1 fruits
        for (int x : basket2) count2[x]++;//count for basket 2 fruits
